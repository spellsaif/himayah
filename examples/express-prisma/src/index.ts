...
// Build Password hashing storage plugin options using Prisma model
const pPlugin = passwordPlugin({
  getPasswordHash: async (userId) => {
    const row = await prisma.passwordHash.findUnique({
      where: { userId }
    });
    return row?.hash || null;
  },
  setPasswordHash: async (userId, hash) => {
    await prisma.passwordHash.upsert({
      where: { userId },
      update: { hash },
      create: { userId, hash }
    });
...
app.post("/api/dev/mailbox/clear", (req, res) => {
  devMailbox.length = 0;
  res.json({ ok: true });
});

// Inject Himayah Express Middleware
...
<head>
  ...
  <title>Himayah Auth Dashboard (Express)</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
  <style>
    :root {
      --bg-gradient: radial-gradient(circle at 50% 0%, #0c1220 0%, #03060b 100%);
      --card-bg: rgba(10, 15, 28, 0.75);
...
<body>
  <header>
    <div class="logo-container">
      ...
      <span class="logo-subtitle">Express & Prisma</span>
    </div>
